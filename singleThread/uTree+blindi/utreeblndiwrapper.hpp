...
void Wrapper::list_node_t::printAll(void) { 
...
#else
  void *ret = Wrapper::curr_addr;
  memset(ret, 0, sizeof(Wrapper::list_node_t));
  Wrapper::curr_addr += size;
  if (Wrapper::curr_addr >= Wrapper::start_addr + Wrapper::SPACE_PER_THREAD) {
    printf("start_addr is %p, curr_addr is %p, SPACE_PER_THREAD is %lu, no "
           "free space to alloc\n",
           Wrapper::start_addr, Wrapper::curr_addr, Wrapper::SPACE_PER_THREAD);
    exit(0);
  }
  return ret;
#endif
...
class btreeWrapper{
  ...
  public:
    Wrapper::list_node_t *list_head = NULL;
    btreeWrapper();
    ~btreeWrapper();
    void setNewRoot(char *);//dc
    void getNumberOfNodes();
    void btree_insert_pred(Wrapper::entry_ket_t, char*, char **pred, bool*);
    void btree_insert_internal(char *, Wrapper::entry_ket_t, char *, uint32_t);
    void btree_delete(Wrapper::entry_ket_t);
    char *btree_search(Wrapper::entry_ket_t);
    char *btree_search_pred(Wrapper::entry_ket_t, bool *f, char**, bool);
    void printAll();
    void insert(Wrapper::entry_ket_t, char*); // Insert
    void remove(Wrapper::entry_ket_t);        // Remove
    char* search(Wrapper::entry_ket_t);       // Search
    void scan(Wrapper::entry_ket_t key, int num, uint64_t buf[]);

    void print()
    {
      int i = 0;
      Wrapper::list_node_t *tmp = list_head;
      while (tmp->next != NULL) {
    ...
    std::array<uint8_t, key_size> int64ToArray(int64_t value)
    {
    ...
    friend class Wrapper::page;
};


class header{
  private:
    Wrapper::page* leftmost_ptr;         // 8 bytes
    Wrapper::page* sibling_ptr;          // 8 bytes
    Wrapper::page* pred_ptr;             // 8 bytes
...
class entry{ 
  private:
    Wrapper::entry_ket_t key; // 8 bytes
    char* ptr; // 8 bytes
...
namespace Wrapper{
  ...
  public:
    ...
    page(page* left, Wrapper::entry_ket_t key, page* right, uint32_t level = 0) {
    ...
    inline bool remove_key(Wrapper::entry_ket_t key) {
    ...
    bool remove(btreeWrapper* bt, Wrapper::entry_ket_t key, bool only_rebalance = false, bool with_lock = true) {
      hdr.mtx->lock();
    ...
    inline void insert_key(Wrapper::entry_ket_t key, char* ptr, int *num_entries, bool flush = true,
        bool update_last_index = true) {
          ...
          if(*num_entries == 0) {  // this page is empty
            ...
            new_entry->key = (Wrapper::entry_ket_t) key;
    ...
    page *store(btreeWrapper* bt, char* left, Wrapper::entry_ket_t key, char* right,
       bool flush, bool with_lock, page *invalid_sibling = NULL) {
        ...
        if(num_entries < cardinality - 1) {
          ...
          Wrapper::entry_ket_t split_key = records[m].key;
    ...
    // revised 
    inline void insert_key(Wrapper::entry_ket_t key, char* ptr, int *num_entries, char **pred, bool flush = true,
          bool update_last_index = true) {
        // update switch_counter
        if(!IS_FORWARD(hdr.switch_counter))
          ++hdr.switch_counter;
    ...
    char *linear_search(Wrapper::entry_ket_t key) {
      int i = 1;
      uint8_t previous_switch_counter;
      char *ret = NULL;
      char *t; 
      Wrapper::entry_ket_t k;
    ...
    char *linear_search_pred(Wrapper::entry_ket_t key, char **pred, bool debug=false) {
      int i = 1;
      uint8_t previous_switch_counter;
      char *ret = NULL;
      char *t; 
      Wrapper::entry_ket_t k, k1;
...
btreeWrapper::btreeWrapper(){
  ...
  printf("without pmdk!\n");
#endif
  root = (char*)new Wrapper::page();
  list_head = (Wrapper::list_node_t *)alloc(sizeof(Wrapper::list_node_t));
  printf("list_head=%p\n", list_head);
  list_head->next = NULL;
  height = 1;
...
void btreeWrapper::setNewRoot(char *new_root) {
  this->root = (char*)new_root;
  ++height;
}

char *btreeWrapper::btree_search_pred(Wrapper::entry_ket_t key, bool *f, char **prev, bool debug=false){
  Wrapper::page* p = (Wrapper::page*)root;

  while(p->hdr.leftmost_ptr != NULL) {
    p = (Wrapper::page *)p->linear_search(key);
  }

  Wrapper::page *t;
  while((t = (Wrapper::page *)p->linear_search_pred(key, prev, debug)) == p->hdr.sibling_ptr) {
    p = t;
    if(!p) {
      break;
    }
...
char *btreeWrapper::search(Wrapper::entry_ket_t key) {
  //search in btree bt
  return *bt.search(int64ToArray(key));
}

void btreeWrapper::scan(Wrapper::entry_ket_t key, int num, uint64_t buf[]){
  bool f = false;
  char *prev;
  char *ptr = btree_search_pred(key, &f, &prev);
  register int i;
  assert(f);
  Wrapper::list_node_t *n = (Wrapper::list_node_t *)ptr;
  for (i = 0; i < num; i++) {
    buf[i] = n->ptr;
...
void btreeWrapper::btree_insert_pred(Wrapper::entry_ket_t key, char* right, char **pred, bool *update){ //need to be string
  Wrapper::page* p = (Wrapper::page*)root;
...
void btreeWrapper::insert(Wrapper::entry_ket_t key, char *right) {
  //insert to btree bt
  bt.insert(int64ToArray(key), right);
}


void btreeWrapper::remove(Wrapper::entry_ket_t key) {
  //remove from btree bt
  bt.remove(int64ToArray(key));
}

// store the key into the node at the given level 
void btreeWrapper::btree_insert_internal(char *left, Wrapper::entry_ket_t key, char *right, uint32_t level) {
  if(level > ((Wrapper::page *)root)->hdr.level)
    return;

  Wrapper::page *p = (Wrapper::page *)this->root;
  ...
  //insert internal node to btree bt
  //bt.btree_insert_internal(int64ToArray(key), right, level);
}

void btreeWrapper::btree_delete(Wrapper::entry_ket_t key) {
  Wrapper::page* p = (Wrapper::page*)root;
  ...
  Wrapper::page *t;
  while((t = (Wrapper::page *)p->linear_search(key)) == p->hdr.sibling_ptr) {
    p = t;
    if(!p)
      break;
  }
  ...
  //delete from btree bt
  bt.remove(int64ToArray(key));
}

void btreeWrapper::printAll(){
  pthread_mutex_lock(&Wrapper::print_mtx);
  int total_keys = 0;
  Wrapper::page *leftmost = (Wrapper::page *)root;
  printf("root: %x\n", root);
  do {
    Wrapper::page *sibling = leftmost;
    while(sibling) {
  ...
  pthread_mutex_unlock(&Wrapper::print_mtx);
}
