...
char * Wrapper::start_addr;
char * Wrapper::curr_addr;
...
int main()
{
    ...
    Wrapper::start_addr = (char *)pmem;
    Wrapper::curr_addr = Wrapper::start_addr;
    printf("start_addr=%p, end_addr=%p\n", Wrapper::start_addr,
           Wrapper::start_addr + (uint64_t)600ULL * 1024ULL * 1024ULL * 1024ULL);
#endif
    btreeWrapper *bt;
    bt = new btreeWrapper();
	srand((int)time(0));
